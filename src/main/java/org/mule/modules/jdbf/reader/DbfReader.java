...
public class DbfReader implements Closeable {
    ...
    public DbfReader(InputStream dbfInputStream) throws IOException {
        this.dbfInputStream = new BufferedInputStream(dbfInputStream, Integer.MAX_VALUE - 8);
        readMetadata();
    }
    ...
    public void goToRow(int row) throws IOException {
        dbfInputStream.reset();
        int fullHeaderLength = metadata.getFullHeaderLength();
        int position = fullHeaderLength + row * metadata.getOneRecordLength();

        int timesToIterate = position / 2048;

        for (int i = 0; i < timesToIterate; i++) {
            dbfInputStream.skip(2048);
        }

        int i = timesToIterate * 2048;
        int remaining = position - (i < 0 ? 0 : i);
        dbfInputStream.skip(remaining);
    }
